    ...
    int vectors_x[NdiaB][MdiaB],int vectors_y[NdiaB][MdiaB])
{
  ...
  for(x=0; x<NdiaB; x++)
  {
    for(y=0; y<MdiaB; y++)
    {
      ...
      int B_x = Bx*x;
      int B_y = By*y;
      int vector_Bx = vector_x + B_x;
      int vector_By = vector_y + B_y;
      ...
      vectors_x[x][y] = vector_Bx - B_x;
      vectors_y[x][y] = vector_By - B_y;
    }
  }
}
...
{
  Bx = atoi(argv[1]); // Bx, By as inputs
  By = atoi(argv[2]);
  NdiaB = N/Bx; // extract time-consuming calculations
  MdiaB = M/By; 
  bigInt = 255 << 8; // shift is faster than multiplication (*16*16)
